...
def GridIntersectionPoints(g, xLen, yLen):
    points = []
    for i in range(xLen):
        for j in range(yLen):
            if g[i][j] == 3:
...
def ManhattenDistance(g, xLen, yLen):
    # Don't have a grid over 4 million in any direction
    min_dist = 4000000
    # Find the points of intersection
    intersect_points = GridIntersectionPoints(g, xLen, yLen)
