...
# Calculates the number of steps required to get to a given point
# Discounts loops in the circuit in the calculation per Problem 3 Pt 2
# path - list of points
# point - point to calculate steps to
# Returns number of steps
def CalculateSteps(path, point):
    step_table = {}
    step_table
    steps = 0
    for p in path:
        if(p == point):
            steps += 1
            break
        if p not in step_table:
            steps += 1
            step_table[p] = steps
        else:
            #print("Loop point: {0}".format(p))
            steps = step_table[p]
    return steps

def CalculateStepsNoLoop(path, point):
    step_table = {}
    steps = 0
    for p in path:
        steps += 1
...
print()
if DEBUG:
    print("[*] Computing number of steps for each intersection")
    for intersection in intersectionPoints:
        print("Steps for pathOne point {0} is: {1}".format(intersection, CalculateSteps(firstPath, intersection)))
    for intersection in intersectionPoints:
        print("Steps for pathTwo point {0} is: {1}".format(intersection, CalculateSteps(secondPath, intersection)))
print("[*] Calculating minimum steps for the circuits")
min_steps = 4000000
step_table = {}
#print("Path One: {0}".format(firstPath))
#print("Path Two: {0}".format(secondPath))
for i in intersectionPoints:
    one = CalculateStepsNoLoop(firstPath, i)
    two = CalculateStepsNoLoop(secondPath, i)
    step_table[i] = [one, two, one+two]
print("")
test = {}
# Find the minimum steps
for k,v in step_table.items():
    min_steps = min(min_steps, v[0]+v[1])
    test[min_steps] = k
print("Minimum steps for the paths is: {0} for point: {1}".format(min_steps, test[min_steps]))
